...
class Vietlott645Controller extends ControllerBase
{
    /**
    ...
    public function index645Action()
    {
        ...
        if ($cachedResult !== null) {
            ...
            // Schema data for Vietlott 645 page (cache hit)
            $breadcrumbs = [
                ['name' => 'Trang chủ', 'url' => $this->getAbsoluteUrl('/')],
                ['name' => 'Mega 6/45', 'url' => $this->getAbsoluteUrl()]
            ];
            ...
            if (!empty($cachedResult['selectedDate'])) {
                $breadcrumbs = [
                    ['name' => 'Trang chủ', 'url' => $this->getAbsoluteUrl('/')],
                    ['name' => 'Mega 6/45', 'url' => $this->getAbsoluteUrl('/ket-qua-xoso-mega-6-45-vietlott')],
            ...
            $this->view->setVar('page_schema_data', [
                'title' => $cachedResult['seo_title'] ?? 'Mega 6/45 - Kết quả xổ số Vietlott',
                'description' => $cachedResult['seo_description'] ?? 'Kết quả xổ số Mega 6/45 Vietlott mới nhất',
        ...
        // Lấy kỳ quay mới nhất của Mega 6/45
        ...
        $finalResult = [
            ...
            'currentPage' => 1,
            'baseUrl' => '/ket-qua-xoso-mega-6-45-vietlott.html'
        ];
        
        // Generate SEO data using SeoHelper
        $seoData = \App\Library\SeoHelper::generateVietlottMeta($drawType, $latestResult['draw_date'] ?? null);
        ...
        // Schema data for Vietlott 645 page
        $this->view->setVar('page_schema_type', 'webpage');
        $this->view->setVar('page_schema_data', [
            'title' => $finalResult['seo_title'] ?? 'Mega 6/45 - Kết quả xổ số Vietlott',
            'description' => $finalResult['seo_description'] ?? 'Kết quả xổ số Mega 6/45 Vietlott mới nhất',
            'url' => $this->getAbsoluteUrl(),
            'breadcrumbs' => [
                ['name' => 'Trang chủ', 'url' => $this->getAbsoluteUrl('/')],
                ['name' => 'Mega 6/45', 'url' => $this->getAbsoluteUrl()]
        ...
        $this->view->pick('vietlott645/index645');
    }
    ...
    private function handleSpecificDate($date, $drawType)
    {
        ...
        if (!$specificResult) {
            ...
            return $this->response->redirect('/ket-qua-xoso-mega-6-45-vietlott.html');
        }
        ...
        $finalResult = [
            ...
            'jackpotIncrease' => $jackpotIncrease,
            'currentPage' => 1,
            'baseUrl' => '/ket-qua-xoso-mega-6-45-vietlott',
            'selectedDate' => $date,
            'isSpecificDate' => true,
