...
class VietlottmaxController extends ControllerBase{
    /**
    ...
    public function index3dmaxAction(?string $date = null)
    {
        ...
        if ($cachedResult !== null) {
            ...
            // Schema data for Max 3D page (cache hit)
            $breadcrumbs = [
                ['name' => 'Trang chủ', 'url' => $this->getAbsoluteUrl('/')],
                ['name' => 'Max 3D', 'url' => $this->getAbsoluteUrl()]
            ];
            ...
            if (!empty($cachedResult['selectedDate'])) {
                $breadcrumbs = [
                    ['name' => 'Trang chủ', 'url' => $this->getAbsoluteUrl('/')],
                    ['name' => 'Max 3D', 'url' => $this->getAbsoluteUrl('/ket-qua-xoso-max3d-vietlott')],
            ...
            $this->view->setVar('page_schema_data', [
                'title' => $cachedResult['seo_title'] ?? 'Max 3D - Kết quả xổ số Vietlott',
                'description' => $cachedResult['seo_description'] ?? 'Kết quả xổ số Max 3D Vietlott mới nhất',
        ...
        // Lấy kết quả gần đây - chỉ lấy 1 kết quả mới nhất
        ...
        // Schema data for Max 3D page (cache miss)
        $breadcrumbs = [
            ['name' => 'Trang chủ', 'url' => $this->getAbsoluteUrl('/')],
            ['name' => 'Max 3D', 'url' => $this->getAbsoluteUrl()]
        ];
        ...
        $this->view->setVar('page_schema_type', 'webpage');
        $this->view->setVar('page_schema_data', [
            'title' => $finalResult['seo_title'] ?? 'Max 3D - Kết quả xổ số Vietlott',
            'description' => $finalResult['seo_description'] ?? 'Kết quả xổ số Max 3D Vietlott mới nhất',
            'url' => $this->getAbsoluteUrl(),
            'breadcrumbs' => $breadcrumbs
        ]);
    }
