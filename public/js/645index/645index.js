document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderMainResult(result) {
    if (!result) {
      resultContent.innerHTML =
        '<p class="text-center text-danger">Không tìm thấy kết quả cho ngày đã chọn.</p>';
      return;
    }
    ...
    const html = `
            <h3 class="text-center fw-bold fs-6 mb-3">Kỳ quay thưởng <span class="text-warning">${
              result.draw_number
      ...
      year: "numeric",
            ...
            <div class="balls justify-content-center mega-645 js-result-number" data-mega="${
              result.id
            }">
                ${mainNumbers
                  .map(
                    (number) =>
                      `<span class="ball mega big mx-1">${number.padStart(
                        2,
                        "0"
                      )}</span>`
                  )
            ...
            <div class="mt-3 mb-3">
                Jackpot: <span class="fw-bold fs-1 text-red d-block lh-1 js_mega_jackpot">${new Intl.NumberFormat(
                  "vi-VN"
                ).format(result.jackpot_amount)}₫</span>
            </div>
                ...
                <table class="table table-bordered text-start align-middle small vietlott__winners__table bg-dark-vietlott">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="text-end text-white">${new Intl.NumberFormat(
                              "vi-VN"
                            ).format(result.jackpot_amount)}₫</td>
                        </tr>
                        <tr>
                            <td>Giải nhất</td>
                            <td><span class="small text-orange">●●●●●</span></td>
                            ...
                            <td class="text-end text-white">${new Intl.NumberFormat(
                              "vi-VN"
                            ).format(result.giai_nhat_gia_tri)}₫</td>
                        </tr>
                        <tr>
                            <td>Giải nhì</td>
                            <td><span class="small text-orange">●●●●</span></td>
                            ...
                            <td class="text-end text-white">${new Intl.NumberFormat(
                              "vi-VN"
                            ).format(result.giai_nhi_gia_tri)}₫</td>
                        </tr>
                        <tr>
                            <td>Giải ba</td>
                            <td><span class="small text-orange">●●●</span></td>
                            ...
                            <td class="text-end text-white">${new Intl.NumberFormat(
                              "vi-VN"
                            ).format(result.giai_ba_gia_tri)}₫</td>
                        </tr>
                    </tbody>
                </table>
            </div>
  ...
  function renderRecentResults(results) {
    ...
    if (!results || results.length === 0) {
      resultsContainer.innerHTML =
        '<tr><td colspan="3" class="text-center">Không có kết quả để hiển thị.</td></tr>';
      return;
    }
    ...
    results.forEach((result) => {
      ...
      const html = `
                <tr>
                    <td>
                        <a title="Kết quả xổ số Vietlott Mega 6/45 ngày ${new Date(
                          result.draw_date
                        ).toLocaleDateString("vi-VN")}"
                           href="/ket-qua-xoso-mega-6-45-vietlott-${new Date(
                             result.draw_date
                           )
                             .toLocaleDateString("vi-VN")
                             .replace(/\//g, "-")}"
                           class="text-decoration-none js-mega-date-link"
                           data-date="${new Date(result.draw_date)
                             .toISOString()
                             .slice(0, 10)}">
                    ...
                    <td>
                        <div class="balls">
                            ${mainNumbers
                              .map(
                                (number) =>
                                  `<span class="ball mega me-1">${number.padStart(
                                    2,
                                    "0"
                                  )}</span>`
                              )
                    ...
                    <td class="text-end"><span class="text-nowrap">${new Intl.NumberFormat(
                      "vi-VN"
                    ).format(result.jackpot_amount)}</span></td>
                </tr>
            `;
  ...
  function handleDateSelection(selectedDate) {
    fetch(`/vietlott645/byDate?date=${selectedDate}`, {
      method: "GET",
      headers: {
      ...
      .then((data) => {
        if (data.status === "success" && data.data) {
          ...
          const formattedDate = `${day}-${month}-${year}`;
          const newUrl = `/ket-qua-xoso-mega-6-45-vietlott-${formattedDate}`;
          
          // Update breadcrumb and title
          updateBreadcrumbAndTitle(selectedDate, 'Mega 6/45');
        
          window.history.pushState({ path: newUrl }, "", newUrl);
          renderMainResult(data.data);
          fetch(`/vietlott645/results?page=1&limit=6`, {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
            },
  ...
  const loadMoreBtn = document.querySelector(".js-btn-load-more-mega");
  ...
  if (loadMoreBtn && resultsContainer) {
    loadMoreBtn.addEventListener("click", function () {
      ...
      fetch(`/vietlott645/results?page=${page}&limit=${limit}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
        ...
        .then((data) => {
          if (data.status === "success" && data.data.length > 0) {
            data.data.forEach((result) => {
              ...
              <tr>
                <td>
                  <a title="Kết quả xổ số Vietlott Mega 6/45 ngày ${new Date(result.draw_date).toLocaleDateString("vi-VN")}"
                    href="/ket-qua-xoso-mega-6-45-vietlott-${new Date(result.draw_date).toLocaleDateString("vi-VN").replace(/\//g, "-")}"
                ...
                <td>
                  <div class="balls">
                    ${mainNumbers.map(
                        (number) =>`<span class="ball mega me-1">${number.padStart(2,"0")}</span>`).join("")}
                  </div>
                </td>
                <td class="text-end"><span class="text-nowrap">${new Intl.NumberFormat(
                  "vi-VN"
  ...
  document.addEventListener("click", function (event) {
    const link = event.target.closest(".js-mega-date-link");
    if (link) {
