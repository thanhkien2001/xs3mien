...
while (time() - $start < 2100) { // Chạy tối đa 35 phút
    ...
    $fetchUrl = $liveDomain . '/xstt/js_m3.js?_=' . time();
    ...
    $data = parseJsonMinhNgoc($response['body'], 'kqxs.mt=');
    ...
    if ($currentData !== $lastData) {
        try {
            $redis->setex('xs_live_mt_' . date('Y-m-d'), 3600, $currentData); // TTL 24 giờ
...
if ($flagDone) {
    ...
    try {
        ...
        $redis->setex('xs_live_mt_' . date('Y-m-d'), 3600, json_encode($resultsData));
    ...
    require_once __DIR__ . '/cache_update_mt.php';
